...
import { Heart, MapPin, BedDouble, Bath, Maximize2, Zap, Gauge, Shield, ArrowUpRight, Scale, Check } from 'lucide-react';
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className={`px-3.5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
              isCompared
                ? 'bg-amber-100/90 dark:bg-amber-950/90 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-800'
                : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
          >
            {isCompared ? <Check className="w-4 h-4 text-amber-600 dark:text-amber-400" /> : <Scale className="w-4 h-4" />}
          ...
          <button
            onClick={() => onSelect(item)}
            className="btn-primary py-2.5 px-4.5 text-xs font-bold"
          >
